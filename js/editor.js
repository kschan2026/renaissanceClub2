...
export function initEditor() {
  ...
  dom.poster.addEventListener('input', event => {
    ...
    updateState(state => {
      ...
      if (field === 'clubName' || field === 'teacherName') state[field] = value;
        ...
        let photo = state.photos.find(p => p.slotId === el.dataset.slotId);
        if (!photo) { photo = {slotId: el.dataset.slotId, dataUrl: '', caption: '', crop: {x:50,y:50,scale:1}}; state.photos.push(photo); }
        photo.caption = value;
      }
    });
    setSaveStatus('저장하지 않은 변경 사항');
